from rpgram.boosters.classe import Classe
from rpgram.boosters.equipment import Equipment
from rpgram.boosters.race import Race
from rpgram.characters.char_base import BaseCharacter
from rpgram.conditions.heal import HealingCondition
from rpgram.consumables.heal import HealingConsumable
from rpgram.dice import Dice
from rpgram.enums.classe import ClasseEnum
from rpgram.enums.damage import DamageEnum
from rpgram.enums.equipment import EquipmentEnum
from rpgram.enums.race import RaceEnum
from rpgram.enums.turn import TurnEnum
from rpgram.equips import Equips


STEEL_HELMET = Equipment(
    name='Capacete de Aço',
    equip_type=EquipmentEnum.HELMET,
    damage_types=None,
    weight=10,
    bonus_physical_defense=30,
    bonus_evasion=-5,
)
STEEL_GIANT_SWORD = Equipment(
    name='Espada Gigante de Aço',
    equip_type=EquipmentEnum.TWO_HANDS,
    damage_types=DamageEnum.SLASHING,
    weight=40,
    bonus_physical_attack=30,
    bonus_hit=15,
    bonus_evasion=-10,
)
STEEL_ARMOR = Equipment(
    name='Armadura de Aço',
    equip_type=EquipmentEnum.ARMOR,
    damage_types=None,
    weight=60,
    bonus_physical_defense=80,
    bonus_evasion=-25,
)
LEATHER_BOOTS = Equipment(
    name='Botas de Couro',
    equip_type=EquipmentEnum.BOOTS,
    damage_types=None,
    weight=10,
    bonus_physical_defense=10,
    bonus_magical_defense=10,
    bonus_evasion=30,
)
ANY_RING = Equipment(
    name='Algum Anel',
    equip_type=EquipmentEnum.RING,
    damage_types=None,
    weight=0.1,
    bonus_evasion=100,
)
BRIGHT_AMULET = Equipment(
    name='Colar Brilhante',
    equip_type=EquipmentEnum.AMULET,
    damage_types=None,
    weight=0.2,
    bonus_charisma=150,
)
EQUIPS = Equips(
    player_id=123,
    helmet=STEEL_HELMET,
    left_hand=STEEL_GIANT_SWORD,
    armor=STEEL_ARMOR,
    boots=LEATHER_BOOTS,
    ring=ANY_RING,
    amulet=BRIGHT_AMULET,
)
HUMAN_RACE = Race(
    name=RaceEnum.HUMAN.value,
    description='HUMANO TESTE',
    bonus_strength=15,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.5,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
ORC_RACE = Race(
    name=RaceEnum.ORC.value,
    description=f'{RaceEnum.ORC.value} TESTE'.upper(),
    bonus_strength=15,
    bonus_dexterity=10,
    bonus_constitution=15,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.5,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.5,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
SPECTRUM_RACE = Race(
    name=RaceEnum.SPECTRUM.value,
    description=f'{RaceEnum.SPECTRUM.value} TESTE'.upper(),
    bonus_strength=10,
    bonus_dexterity=20,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.5,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
ARCANIST_CLASSE = Classe(
    name=ClasseEnum.ARCANIST.value,
    description='ARCANISTA TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=20,
    bonus_wisdom=15,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=2.0,
    multiplier_wisdom=1.5,
    multiplier_charisma=1.0,
)
BARBARIAN_CLASSE = Classe(
    name=ClasseEnum.BARBARIAN.value,
    description='BÁRBARO TESTE',
    bonus_strength=15,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.5,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
BARD_CLASSE = Classe(
    name=ClasseEnum.BARD.value,
    description='BARDO TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=15,
    bonus_charisma=20,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.5,
    multiplier_charisma=2.0,
)
BERSERKIR_CLASSE = Classe(
    name=ClasseEnum.BERSERKIR.value,
    description='BERSERKIR TESTE',
    bonus_strength=50,
    bonus_dexterity=25,
    bonus_constitution=40,
    bonus_intelligence=1,
    bonus_wisdom=13,
    bonus_charisma=1,
    multiplier_strength=5.00,
    multiplier_dexterity=2.50,
    multiplier_constitution=4.00,
    multiplier_intelligence=0.10,
    multiplier_wisdom=1.30,
    multiplier_charisma=0.10,
)
BOUNTY_HUNTER_CLASSE = Classe(
    name=ClasseEnum.BOUNTY_HUNTER.value,
    description='CAÇADOR DE RECOMPENSAS TESTE',
    bonus_strength=20,
    bonus_dexterity=15,
    bonus_constitution=15,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=15,
    multiplier_strength=2.0,
    multiplier_dexterity=1.5,
    multiplier_constitution=1.5,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.5,
)
CLERIC_CLASSE = Classe(
    name=ClasseEnum.CLERIC.value,
    description='CLÉRIGO TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=15,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.5,
    multiplier_charisma=1.0,
)
DRUID_CLASSE = Classe(
    name=ClasseEnum.DRUID.value,
    description='DRUIDA TESTE',
    bonus_strength=15,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.5,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
DUELIST_CLASSE = Classe(
    name=ClasseEnum.DUELIST.value,
    description='DUELISTA TESTE',
    bonus_strength=10,
    bonus_dexterity=15,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.5,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
GLADIATOR_CLASSE = Classe(
    name=ClasseEnum.GLADIATOR.value,
    description='GLADIADOR TESTE',
    bonus_strength=20,
    bonus_dexterity=10,
    bonus_constitution=15,
    bonus_intelligence=10,
    bonus_wisdom=15,
    bonus_charisma=15,
    multiplier_strength=2.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.5,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.5,
    multiplier_charisma=1.5,
)
GUARDIAN_CLASSE = Classe(
    name=ClasseEnum.GUARDIAN.value,
    description='GUARDIÃO TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=15,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.5,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
HEALER_CLASSE = Classe(
    name=ClasseEnum.HEALER.value,
    description='CURANDEIRO TESTE',
    bonus_strength=1,
    bonus_dexterity=1,
    bonus_constitution=10,
    bonus_intelligence=23,
    bonus_wisdom=30,
    bonus_charisma=15,
    multiplier_strength=0.10,
    multiplier_dexterity=0.10,
    multiplier_constitution=1.0,
    multiplier_intelligence=2.30,
    multiplier_wisdom=3.00,
    multiplier_charisma=1.50,
)
HERALD_CLASSE = Classe(
    name=ClasseEnum.HERALD.value,
    description='ARAUTO TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=15,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.5,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
KNIGHT_CLASSE = Classe(
    name=ClasseEnum.KNIGHT.value,
    description='CAVALEIRO TESTE',
    bonus_strength=10,
    bonus_dexterity=30,
    bonus_constitution=15,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=15,
    multiplier_strength=1.0,
    multiplier_dexterity=3.0,
    multiplier_constitution=1.5,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.5,
)
LORD_OF_THE_ROGUES_CLASSE = Classe(
    name=ClasseEnum.LORD_OF_THE_ROGUES.value,
    description='SENHOR DOS LADINOS TESTE',
    bonus_strength=20,
    bonus_dexterity=50,
    bonus_constitution=24,
    bonus_intelligence=10,
    bonus_wisdom=24,
    bonus_charisma=2,
    multiplier_strength=2.00,
    multiplier_dexterity=5.00,
    multiplier_constitution=2.40,
    multiplier_intelligence=1.00,
    multiplier_wisdom=2.40,
    multiplier_charisma=0.20,
)
MAGE_CLASSE = Classe(
    name=ClasseEnum.MAGE.value,
    description='MAGO TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=15,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.5,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
MERCENARY_CLASSE = Classe(
    name=ClasseEnum.MERCENARY.value,
    description='MERCENÁRIO TESTE',
    bonus_strength=24,
    bonus_dexterity=15,
    bonus_constitution=25,
    bonus_intelligence=5,
    bonus_wisdom=10,
    bonus_charisma=1,
    multiplier_strength=2.40,
    multiplier_dexterity=1.50,
    multiplier_constitution=2.50,
    multiplier_intelligence=0.50,
    multiplier_wisdom=1.0,
    multiplier_charisma=0.10,
)
NECROMANCER_CLASSE = Classe(
    name=ClasseEnum.NECROMANCER.value,
    description='NECROMANTE TESTE',
    bonus_strength=1,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=33,
    bonus_wisdom=25,
    bonus_charisma=1,
    multiplier_strength=0.10,
    multiplier_dexterity=1.00,
    multiplier_constitution=1.00,
    multiplier_intelligence=3.30,
    multiplier_wisdom=2.50,
    multiplier_charisma=0.10,
)
PALADIN_CLASSE = Classe(
    name=ClasseEnum.PALADIN.value,
    description='PALADINO TESTE',
    bonus_strength=15,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=15,
    bonus_charisma=10,
    multiplier_strength=1.5,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.5,
    multiplier_charisma=1.0,
)
RANGER_CLASSE = Classe(
    name=ClasseEnum.RANGER.value,
    description='PATRULHEIRO TESTE',
    bonus_strength=10,
    bonus_dexterity=30,
    bonus_constitution=20,
    bonus_intelligence=5,
    bonus_wisdom=10,
    bonus_charisma=5,
    multiplier_strength=1.00,
    multiplier_dexterity=3.00,
    multiplier_constitution=2.00,
    multiplier_intelligence=0.50,
    multiplier_wisdom=1.00,
    multiplier_charisma=0.50,
)
ROGUE_CLASSE = Classe(
    name=ClasseEnum.ROGUE.value,
    description='LADINO TESTE',
    bonus_strength=10,
    bonus_dexterity=15,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.5,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
SAMURAI_CLASSE = Classe(
    name=ClasseEnum.SAMURAI.value,
    description='SAMURAI TESTE',
    bonus_strength=5,
    bonus_dexterity=30,
    bonus_constitution=20,
    bonus_intelligence=5,
    bonus_wisdom=5,
    bonus_charisma=15,
    multiplier_strength=0.50,
    multiplier_dexterity=3.00,
    multiplier_constitution=2.00,
    multiplier_intelligence=0.50,
    multiplier_wisdom=0.50,
    multiplier_charisma=1.50,
)
SHAMAN_CLASSE = Classe(
    name=ClasseEnum.SHAMAN.value,
    description='XAMÃ TESTE',
    bonus_strength=1,
    bonus_dexterity=5,
    bonus_constitution=10,
    bonus_intelligence=20,
    bonus_wisdom=30,
    bonus_charisma=14,
    multiplier_strength=0.10,
    multiplier_dexterity=0.50,
    multiplier_constitution=1.0,
    multiplier_intelligence=2.0,
    multiplier_wisdom=3.00,
    multiplier_charisma=1.40,
)
SUMMONER_CLASSE = Classe(
    name=ClasseEnum.SUMMONER.value,
    description='INVOCADOR TESTE',
    bonus_strength=1,
    bonus_dexterity=5,
    bonus_constitution=10,
    bonus_intelligence=40,
    bonus_wisdom=23,
    bonus_charisma=1,
    multiplier_strength=0.10,
    multiplier_dexterity=0.50,
    multiplier_constitution=1.0,
    multiplier_intelligence=4.0,
    multiplier_wisdom=2.30,
    multiplier_charisma=0.10,
)
SORCERER_CLASSE = Classe(
    name=ClasseEnum.SORCERER.value,
    description='FEITICEIRO TESTE',
    bonus_strength=10,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=15,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.0,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.5,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
SORCERER_SUPREME_CLASSE = Classe(
    name=ClasseEnum.SORCERER_SUPREME.value,
    description='FEITICEIRO SUPREMO TESTE',
    bonus_strength=2,
    bonus_dexterity=20,
    bonus_constitution=10,
    bonus_intelligence=40,
    bonus_wisdom=40,
    bonus_charisma=18,
    multiplier_strength=0.20,
    multiplier_dexterity=2.00,
    multiplier_constitution=1.0,
    multiplier_intelligence=4.00,
    multiplier_wisdom=4.00,
    multiplier_charisma=1.80,
)
WARRIOR_CLASSE = Classe(
    name=ClasseEnum.WARRIOR.value,
    description='GUERREIRO TESTE',
    bonus_strength=15,
    bonus_dexterity=10,
    bonus_constitution=10,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=1.5,
    multiplier_dexterity=1.0,
    multiplier_constitution=1.0,
    multiplier_intelligence=1.0,
    multiplier_wisdom=1.0,
    multiplier_charisma=1.0,
)
WEAPON_MASTER_CLASSE = Classe(
    name=ClasseEnum.WEAPON_MASTER.value,
    description='MESTRE DAS ARMAS TESTE',
    bonus_strength=40,
    bonus_dexterity=30,
    bonus_constitution=30,
    bonus_intelligence=10,
    bonus_wisdom=10,
    bonus_charisma=10,
    multiplier_strength=4.00,
    multiplier_dexterity=3.00,
    multiplier_constitution=3.00,
    multiplier_intelligence=1.00,
    multiplier_wisdom=1.00,
    multiplier_charisma=1.00,
)
BASE_CHARACTER = BaseCharacter(
    char_name='PERSONAGEM TESTE',
    classe=WARRIOR_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=21,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
ARCANIST_CHARACTER = BaseCharacter(
    char_name='ARCANISTA TESTE',
    classe=ARCANIST_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
BARBARIAN_CHARACTER = BaseCharacter(
    char_name='BÁRBARO TESTE',
    classe=BARBARIAN_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
BARD_CHARACTER = BaseCharacter(
    char_name='BARDO TESTE',
    classe=BARD_CLASSE,
    race=HUMAN_RACE,
    player_id=10_000,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
BERSERKIR_CHARACTER = BaseCharacter(
    char_name='BERSERKIR TESTE',
    classe=BERSERKIR_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
BOUNTY_HUNTER_CHARACTER = BaseCharacter(
    char_name='BOUNTY_HUNTER TESTE',
    classe=BOUNTY_HUNTER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
CLERIC_CHARACTER = BaseCharacter(
    char_name='CLERIC TESTE',
    classe=CLERIC_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
DRUID_CHARACTER = BaseCharacter(
    char_name='DRUID TESTE',
    classe=DRUID_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
DUELIST_CHARACTER = BaseCharacter(
    char_name='DUELIST TESTE',
    classe=DUELIST_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
GLADIATOR_CHARACTER = BaseCharacter(
    char_name='GLADIATOR TESTE',
    classe=GLADIATOR_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
GUARDIAN_CHARACTER = BaseCharacter(
    char_name='GUARDIAN TESTE',
    classe=GUARDIAN_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
HEALER_CHARACTER = BaseCharacter(
    char_name='HEALER TESTE',
    classe=HEALER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
HERALD_CHARACTER = BaseCharacter(
    char_name='HERALD TESTE',
    classe=HERALD_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
KNIGHT_CHARACTER = BaseCharacter(
    char_name='KNIGHT TESTE',
    classe=KNIGHT_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
LORD_OF_THE_ROGUES_CHARACTER = BaseCharacter(
    char_name='LORD OF THE ROGUES TESTE',
    classe=LORD_OF_THE_ROGUES_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
MAGE_CHARACTER = BaseCharacter(
    char_name='MAGE TESTE',
    classe=MAGE_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
MERCENARY_CHARACTER = BaseCharacter(
    char_name='MERCENARY TESTE',
    classe=MERCENARY_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
NECROMANCER_CHARACTER = BaseCharacter(
    char_name='NECROMANCER TESTE',
    classe=NECROMANCER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
PALADIN_CHARACTER = BaseCharacter(
    char_name='PALADIN TESTE',
    classe=PALADIN_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
RANGER_CHARACTER = BaseCharacter(
    char_name='RANGER TESTE',
    classe=RANGER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
ROGUE_CHARACTER = BaseCharacter(
    char_name='ROGUE TESTE',
    classe=ROGUE_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
SAMURAI_CHARACTER = BaseCharacter(
    char_name='SAMURAI TESTE',
    classe=SAMURAI_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
SHAMAN_CHARACTER = BaseCharacter(
    char_name='SHAMAN TESTE',
    classe=SHAMAN_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
SUMMONER_CHARACTER = BaseCharacter(
    char_name='SUMMONER TESTE',
    classe=SUMMONER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
SORCERER_CHARACTER = BaseCharacter(
    char_name='SORCERER TESTE',
    classe=SORCERER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
SORCERER_SUPREME_CHARACTER = BaseCharacter(
    char_name='SORCERER_SUPREME TESTE',
    classe=SORCERER_SUPREME_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
WARRIOR_CHARACTER = BaseCharacter(
    char_name='WARRIOR TESTE',
    classe=WARRIOR_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
WEAPON_MASTER_CHARACTER = BaseCharacter(
    char_name='WEAPON_MASTER TESTE',
    classe=WEAPON_MASTER_CLASSE,
    race=HUMAN_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
ORC_BARBARIAN_CHARACTER = BaseCharacter(
    char_name='ORQUE BÁRBARO TESTE',
    classe=BARBARIAN_CLASSE,
    race=ORC_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=101,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
SPECTRUM_ROGUE_CHARACTER = BaseCharacter(
    char_name='SPECTRUM ROGUE TESTE',
    classe=ROGUE_CLASSE,
    race=SPECTRUM_RACE,
    equips=EQUIPS,
    _id='ffffffffffffffffffffffff',
    level=151,
    xp=0,
    base_strength=10,
    base_dexterity=10,
    base_constitution=10,
    base_intelligence=10,
    base_wisdom=10,
    base_charisma=10,
    combat_damage=0,
)
POTION = HealingConsumable(
    name='Potion',
    description='Cura 100 de HP.',
    power=100,
    weight=0.1,
    condition=HealingCondition(
        name='Potion',
        power=20,
        frequency=TurnEnum.START,
        turn=5,
        level=1,
    )
)

if __name__ == '__main__':
    print(f'BASE_CHARACTER: {BASE_CHARACTER}')
    print('HP:', BASE_CHARACTER.cs.show_hit_points)
    BASE_CHARACTER.cs.hp = -300
    POTION(BASE_CHARACTER)
    BASE_CHARACTER.cs.hp = -600
    POTION(BASE_CHARACTER)

    dice = Dice(character=BASE_CHARACTER, faces=20)
    dice.throw()
    print(dice.value)
    print(dice.text)
    print(dice.throw())
    print(dice.throw())
    print(dice.throw(rethrow=True))
